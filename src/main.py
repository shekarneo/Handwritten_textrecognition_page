...
def main():
    ...
    # infer text on test image
        ...
        imageg = cv2.cvtColor(cv2.imread(FilePaths.fnInfer), cv2.COLOR_BGR2RGB)
        # imageg = cv2.fastNlMeansDenoisingColored(imageg1,None,20,10,7,21)
        image = cv2.resize(imageg, (1500, 1500))  # 1500,1500
        ...
        for line in lines:
            print(" ".join([infer(model, crop[y1:y2, x1:x2]) for (x1, y1, x2, y2) in line]))
        # for line in lines:
        #     for x1,y1,x2,y2 in line:
        #         infer(model, crop[y1:y2, x1:x2])
        #         #break
